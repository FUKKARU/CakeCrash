...
namespace Main
{
    public class SquatMove : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (!isSquatHeld && isDownMode && GameManager.Instance.IsSquat >= 0.99f && !GameManager.Instance.IsTired && !GameManager.Instance.IsClear && !GameManager.Instance.IsGameOver)
            {
            ...
            if (isSquatHeld && isDownMode)
            {
                ...
                // 疲れていない，かつゲームオーバー状態でもクリア状態でもないときのみ，入力を受け付ける。
                if (GameManager.Instance.IsSquat <= 0.9f && !GameManager.Instance.IsTired && !GameManager.Instance.IsClear && !GameManager.Instance.IsGameOver)
                {
                    releasedY = y;
                    isSquatHeld = false;
                    isDownMode = false;
